...
if ( ! function_exists( 'swpmb_meta' ) )
{
	...
	function swpmb_meta( $key, $args = array(), $post_id = null )
	{
		 ...
		 */
		$field = SWPMB_Helper::find_field( $key, $post_id );
		if ( false === $field || isset( $args['type'] ) )
		{
			return apply_filters( 'swpmb_meta', SWPMB_Helper::meta( $key, $args, $post_id ) );
		}
...
if ( ! function_exists( 'swpmb_get_value' ) )
{
	...
	{
		$field = SWPMB_Helper::find_field( $field_id, $post_id );

		// Get field value
		$value = $field ? call_user_func( array( SWP_Meta_Box::get_class_name( $field ), 'get_value' ), $field, $args, $post_id ) : false;
...
if ( ! function_exists( 'swpmb_the_value' ) )
{
	...
	function swpmb_the_value( $field_id, $args = array(), $post_id = null, $echo = true )
	{
		...
		$output = call_user_func( array( SWP_Meta_Box::get_class_name( $field ), 'the_value' ), $field, $args, $post_id );
...
if ( ! function_exists( 'swpmb_meta_shortcode' ) )
{
	...
	function swpmb_meta_shortcode( $atts )
	{
		...
		return swpmb_the_value( $field_id, $atts, $post_id, false );
	}
